...
export async function generarReporte_02092025_2(tipo_reporte, fecha_reporte, usuario) {
    try {
        ...
        for (const [rubro, recibos] of Object.entries(recibosPorRubro)) {
            ...
            doc.text("Descripción", 85, yPosition);
            ...
            for (const recibo of recibos) {
                ...
                doc.text(recibo.cedula, 55, yPosition);
                doc.text(recibo.descripcion.substring(0, 40), 85, yPosition); // Limitar descripción
    ...
    try {
        ...
        for (const [rubro, recibos] of Object.entries(recibosPorRubro)) {
            ...
            for (const recibo of recibos) {
                ...
                doc.text(recibo.numero_recibo, 15, yPosition);
                doc.text(recibo.forma_pago, 30, yPosition);
                doc.text(recibo.cedula, 50, yPosition);
                doc.text(recibo.descripcion.substring(0, 40), 85, yPosition); // Limitar descripción
                doc.text(formatCurrency(recibo.monto), 170, yPosition, { align: 'right' });
                doc.text(formatCurrency(recibo.monto * recibo.tasa), 190, yPosition, { align: 'right' });
                yPosition += 6;
            }
