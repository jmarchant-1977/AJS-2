...
document.addEventListener('DOMContentLoaded', function() {
    ...
    ['nombre-cliente', 'cedula', 'nombre-est','rubro', 'forma-pago', 'referencia','descripcion', 'monto','periodo'].forEach(id => {
...
// Modificamos updateReciboPreview para que sea async y pueda buscar datos
async function updateReciboPreview() {
    const preview = document.getElementById('recibo-preview');
    const preview_control = document.getElementById('control-preview');
    ...
    // Importante: Obtenemos la cédula del input hidden o del texto si no hay hidden
    ...
    const periodoVal = document.getElementById('periodo').value || '';
    ...
    // 1. Renderizar el Recibo (Igual que antes)
    preview.innerHTML = `
        ...
        <div class="recibo-body">
            ...
            <div class="recibo-row">
                ...
                <span> $ ${monto}</span> -  
                <span> ${parseFloat(parseCurrency(monto) * tasaActual).toLocaleString('es-VE', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>   <span> Bs. </span>
            </div>
        </div>
        <div class="recibo-footer">
            <br>
            <p>¡Educar es nuestra Pasión!</p>
        </div>
    `;

    // 2. Lógica para la Tabla de Control
    preview_control.innerHTML = '<div style="text-align:center; padding:10px;">Cargando control...</div>';
    ...
    if (cedula && periodoVal && periodoVal !== '[Periodo]') {
        try {
            ...
            if (result.success) {
                datosPagos = result.data; // Aquí viene algo tipo { "SEP": 50, "INS": 40 }
            }
        } catch (error) {
            console.error("Error cargando pagos de control:", error);
        }
    }
 ...
 */
function agregarValidacionInputsControl() {
...
async function saveRecibo() {
    ...
    const cedula = document.getElementById('cedula').value || nombre_est;
    ...
    if (!nombre || !rubro || !formaPago || !descripcion|| !referencia || monto <= 0) {
...
function activatePrintButton(numeroRecibo) {
        ...
        try {
            ...
            if (reciboId) {
                ...
                if (typeof imprimirRecibo === 'function') {
                    console.log("Llamando a imprimirRecibo con ID: ", reciboId);
    ...
    newPrintBtn.innerHTML = '<i class="fas fa-print"></i> Imprimir Últ. Recibo:' + numeroRecibo;
}
...
async function imprimirRecibo_recibo_js(reciboId, imprimirDirecto = false) {
...
async function imprimirRecibo(reciboId) {
    ...
    try {
        ...
        console.log("Generando PDF para impresión directa RECIBOS.JS...", recibo);
